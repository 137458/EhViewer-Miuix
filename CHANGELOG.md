...
* 优化图片加载与下载调度管线：SpiderQueen 引入当前激活视口页专属优先并发通道与本地缓存快速出锁检查，杜绝冷启动首图被后台预加载抢占并发槽位导致超时
* 优化 H@H 图床节点测速与容差机制：SpeedTracker 增加 3 秒冷启动免测宽限期与连续 3 次低速判定，并在下载失败重试前增加指数退避延时，杜绝网络瞬时抖动误杀正常图片与重试次数骤死
* 优化阅读器预解码与缺页自愈机制：SpiderQueen 预下载完成联动打通 PageLoader 后台自动预解码，彻底消除翻页白块等待；PagerItem 引入视口停留自动静默重试自愈机制，自动恢复偶发网络抖动导致的缺页
* 优化缩略图磁盘缓存容量：thumbCache 升级为基于设备可用存储空间的自适应动态上限（128MB~512MB），缓解高分屏列表滑动导致的频繁缓存驱逐
...
* 修复画廊图片下载优先并发通道完全绕过信号量导致网络穿透的问题，设立独立优先信号量配额并规范指数退避延时算法
* 修复更新页面与弹窗中部分标题与提示文本未接入多语言资源的问题，补齐规范定义并优化 Markdown 解析器正则分配
* 修复关于页（AboutScreen）中免责声明配置项重复的问题，将“免责声明”与相关说明合并为单项标准 Preference 并规范弹窗触发
* 修复点击“检查更新”打开 UpdateScreen 时因加载 AdaptiveIcon 与实验性 Shader 导致页面运行时崩溃闪退的缺陷，采用安全应用图标位图提取与标准 Miuix Surface 背景
* 修复搜索页材质折射背景异常与历史搜索排版重叠缺陷：在展开搜索页与筛选弹窗中隔离 LocalBackdrop，杜绝穿透透镜采样下层主页卡片内容；将搜索筛选器（SearchFilter）整合进 LazyColumn 列表流 Header 首项，彻底杜绝标签与历史搜索记录的排版重叠与键盘挤压遮挡
...
* 修复下载信息与历史记录删除跨表级联操作缺乏事务包裹的问题，接入 Room 写入连接池原子化保证数据一致性
* 修复清除 ExHentai Cookie（clearIgneous）时域名匹配不一致且遗漏磁盘持久化同步的问题
* 修复下载设置界面将“尝试删除目录”作为非空检查的破坏性逻辑，改为只读非空校验以防误删空目录，并收敛 SAF 权限精准释放
* 修复下载页面网格模式下长按行为与列表模式不一致的问题，统一长按进入多选批处理模式
* 修复阅读器初始化或加载失败陷入全屏死胡同界面的问题，提供重试加载与退出阅读的交互出口
* 修复主界面、下载界面、画廊详情与搜索界面等核心可点击图标缺失无障碍标签（contentDescription）的问题
* 修复多语言字符串资源中历史 warning 拼写错误（waring -> warning），补齐全语种资源定义并保留别名兼容
* 修复设置主页（SettingsScreen）在大标题模式下出现上下双重标题的层级冗余问题，规范对齐 Miuix 排版
* 修复悬浮底部导航栏在手机端遮挡列表与设置页末尾内容的问题，对齐 pixez-flutter-MIUIX 架构规范引入 LocalBottomBarContentPadding 全局避让与动态外边距计算
* 修复宽屏与横屏场景下侧边导航栏（NavigationRail）缺失状态栏与摄像头挖孔安全区避让（safeDrawing）的问题

* 修复网络请求响应体超过 512KB 时直接内存缓冲区耗尽陷入 100% CPU 忙等死循环的问题，支持最高 16MB 动态安全扩容与超限拦截
* 修复下载标签删除未命中时触发的数组越界崩溃，同步持久化后续标签位移至本地数据库
* 修复下载界面标签拖动排序跨越前置固定项时引发的数组越界异常，增加范围边界防御
* 修复底层归档库中 read 系统调用返回值有符号转换导致的段错误崩溃隐患，重构 ZIP 归档打包为流式单文件即时释放机制以根治文件描述符（fd）耗尽闪退
* 修复静态类加载期访问系统 WebViewCompat 失败时引发的空指针崩溃，改为安全惰性求值与回退机制
* 修复分块传输编码（Chunked）下下载速度跟踪器由于强制非空断言引发的空指针崩溃，并补齐 HTTP 509 带宽配额超限异常精确识别
* 修复悬浮底部导航栏（FloatingBottomBar）指示器在浅色模式下白底画白完全隐形的问题，对齐微晶黑透表面着色与物理内阴影规范
* 修复底栏尺寸约束与项目宽度分配机制，改用固有尺寸测量与高精度像素采集，消除高分屏下子像素舍入抖动与位置偏差
* 修复底栏拖拽手势拦截与点击穿透判定，增加边界条件限制与点击容差阈值，支持当前选中项点击刷新与父级手势正常分发
* 修复宿主容器背景采样在页面过渡时出现黑边噪点的问题，规范接入带底色保护的毛玻璃采样源（rememberBlurBackdrop）并补全无障碍与触感反馈
* 修复双击底栏同一个 Tab 触发重入导致界面向左上角位移错位且底栏变灰的动画重入问题，增加导航防抖与当前目的地幂等校验
* 修复首页右下角悬浮玻璃操作胶囊内部图标垂直居中对齐偏上的几何偏差
* 修复顶栏渐进式纹理模糊高度固定且透明度过高的问题，支持随滚动动态计算过渡高度并优化磨砂玻璃质感
...
* 修复悬浮底部导航栏遮挡 Snackbar 提示的问题
* 修复网页设置页（UConfig）保存操作的异步时序竞态
* 修复画廊详情页“屏蔽上传者”与错误重试入口缺失无障碍名称、读屏无法识别的问题
* 修复画廊列表加载失败时重试入口可被重复触发且无禁用态的问题，改为标准重试按钮并增加并发防连击控制
* 修复阅读器设置开关同时存在整行与开关两处点击区域的问题
* 修复阅读器背景主题以纯白颜色值判定明暗、非纯白主题下明暗与状态栏图标判断失效的问题
* 修复滑动删除历史记录、下载标签与快速搜索时缺少可见删除提示的问题，抽象统一滑动删除背景与无障碍辅助操作（dismissDeleteAction）
* 修复选择类列表项与偏好项缺少角色与选中状态语义（triStateToggleable / selectable）、读屏无法正确播报的问题
* 修复下载卡片操作按钮、文本框清除按钮与标签管理按钮缺失无障碍文本描述的问题
* 解耦搜索建议项整行与删除按钮的父子嵌套点击区域，优化画廊详情上传者入口触控高度与按钮角色声明

### 重构

* 完成画廊评论页（GalleryCommentsScreen）全面迁移至原生 Miuix 规范（Scaffold、TopAppBar、BlurredBar、PullToRefresh）
...
* 对话框选择行、种子列表行与阅读器开关迁移至 Miuix BasicComponent 与 SwitchPreference，统一行高、按压反馈与语义
* 选择类列表项底色改用 Miuix 语义容器令牌（primaryContainer / onPrimaryContainer），移除透明度合成色
* 侧边抽屉遮罩改用 Miuix windowDimming 语义令牌，标签弱化态改用次级文本令牌

### 优化

* 适配桌面与启动图标为 HyperOS 科技蓝与天水蓝双色方案
* 适配 Android TV 横幅与抽屉界面横幅为科技蓝与天水蓝品牌排版方案
* 统一宽屏侧边栏及抽屉头部使用新版蓝白品牌图标与横幅

### 文档

* 重写项目 README 文档以全面对齐 MIUIX 设计系统特性，补全防伪声明、FAQ 与致谢名单
* 替换根目录与多语言 README 头部为本地蓝白品牌矢量素材，彻底移除失效外链
* 补全正体中文（zh-tw）与日文（ja）README 文档特性内容与规范对齐
