...
- 添加了全新的检查更新页面与更新弹窗：支持更新详情展示、实时下载速率与进度显示、更新通道管理、版本忽略机制与更新弹窗沙盒模拟测试。
...
- 优化了图片加载与下载调度管线：当前浏览视口优先占用并发通道并加速本地缓存命中，避免翻页首图被后台预加载抢占。
- 优化了图床节点测速与容差重试机制：增加冷启动免测宽限期与指数退避重试延时，减少网络瞬时抖动导致的加载失败。
- 优化了阅读器图片预解码与缺页恢复体验：后台自动预解码下一页以消除翻页白块等待，并在当前页遇网络波动缺页时自动静默重试恢复。
